...
SELECT gender, AVG(salary) AS 'Average Salary', COUNT(*) as 'Count' FROM users
GROUP BY gender;
...
SELECT gender, AVG(salary) AS 'Average Salary', COUNT(*) as 'Count' 
FROM users WHERE id < 5000 GROUP BY gender
HAVING AVG(salary) > 61000 AND COUNT(*)<4000;


-- Find those nama who is referred by someone else.
