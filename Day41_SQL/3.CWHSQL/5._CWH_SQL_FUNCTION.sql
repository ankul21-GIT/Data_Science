...
SELECT gender, AVG(salary) avg_salary FROM users GROUP BY gender; 

SELECT gender, SUM(salary) sum_salary FROM users GROUP BY gender;


SELECT id, gender, LOWER(name) AS lower , LENGTH(name) len_of_name FROM users;

SELECT id, gender, LOWER(name) AS lower ,CONCAT (LOWER(name), '5677') AS user_name, LENGTH(name) len_of_name FROM users;


SELECT id, gender, LOWER(name) AS lower ,CONCAT (LOWER(name), '5677') AS user_name, NOW() AS time, LENGTH(name) len_of_name FROM users;

-- For year

SELECT id, gender, LOWER(name) AS lower ,CONCAT (LOWER(name), '5677') AS user_name, YEAR(NOW()) AS time, LENGTH(name) len_of_name FROM users;
-- --- OR
SELECT id, gender,
LOWER(name) AS lower,
CONCAT (LOWER(name), '5677') AS user_name, 
YEAR(NOW()) AS time, 
LENGTH(name) len_of_name FROM users;
...
SELECT salary,
    ...
    FLOOR(salary) AS flored,
    CEIL(salary) AS ceild
FROM users;
