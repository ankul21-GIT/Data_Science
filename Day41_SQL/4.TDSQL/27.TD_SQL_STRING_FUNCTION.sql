...
-- 2. Mask part of phone number for privacy.

SELECT CONCAT(LEFT('9876543210',3), '*****', RIGHT('9876543210', 2)) AS masked_phone;

-- 3. Genrate a unique referance code.

SELECT CONCAT('INV-', LPAD(12345, 6, '0')) AS invoice_code;

SELECT CONCAT('INV-', LPAD(12345, 7, '0')) AS invoice_code;
...
SELECT LOWER(REPLACE('My Blog Title', ' ', '-')) AS url_slug;
...
SELECT (LENGTH('apple banana apple') - LENGTH(REPLACE('apple banana apple', 'apple', ''))) / LENGTH('apple') AS count;
...
SELECT SUBSTRING_INDEX(empName, ' ', 1) AS employeeName, COUNT(*)
FROM employees
GROUP BY empName
...
SELECT REGEXP_REPLACE('<b>Hello</b>', '<[^>]*>', '') AS clean_text;
